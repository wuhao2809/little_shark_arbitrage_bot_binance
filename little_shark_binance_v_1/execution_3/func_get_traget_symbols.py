...
def store_price_history_static(symbols: list) -> str:
    ...
    price_history_dict = {}
    for sym in symbols:
        price_history = binance_get_recent_close_price(sym, interval=INTERVAL, limit=NUM_INTERVAL_LIMIT + SPREAD_WINDOW + Z_SCORE_WINDOW)
        if len(price_history) == NUM_INTERVAL_LIMIT + SPREAD_WINDOW + Z_SCORE_WINDOW: # make sure that each symbol has the same amount of data
...
def get_cointegrated_pairs_dynamic(prices, df_coint_static, num_wave=0):
    ...
    for i in range(len(sym_1_list)):
        ...
        series_1_spread_test = series_1[- (BACKTEST_INTERVAL + SPREAD_WINDOW + Z_SCORE_WINDOW):]
        series_2_spread_test = series_2[- (BACKTEST_INTERVAL + SPREAD_WINDOW + Z_SCORE_WINDOW):]
        
        # derive the dynamic_hedge_ratio_spread length is 2 * z_score_window + BACKTEST_INTERVAL
        spread_list, hedge_ratio_list = calculate_spread_hedge_ratio_window(series_1_spread_test, series_2_spread_test, SPREAD_WINDOW)
