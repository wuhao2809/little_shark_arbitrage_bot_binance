...
from config import ONBOARD_TIME_THRESHOD, TRADING_VOLUME_THRESHOD_RATE, INTERVAL, NUM_INTERVAL_LIMIT, TRIGGER_Z_SCORE_THRESHOD, Z_SCORE_WINDOW, TRADING_TIMES_THRESHOD, INVESTIBLE_CAPITAL_EACH_TIME, TRADING_FEE_RATE, WIN_RATE_THRESHOD, EXTREME_VALUE_MEAN_RATE_THRESHOD, STOP_LOSS_RATIO, LEVERAGE
...
from func_calculation_static import calculate_cointegration_static, calculate_spread_static, calculate_z_score_window, calculate_std_spread
...
def choose_best_trading_pair_static(df_coint: pd.DataFrame) ->tuple:
    ...
    mean = df_coint["estimated_returns"].mean()
    df_coint = df_coint[df_coint["estimated_returns"] < EXTREME_VALUE_MEAN_RATE_THRESHOD * mean]
    ...
    # get the pairs larger than the average
    df_coint = df_coint[df_coint["estimated_returns"] > mean]
    ...
    for i in range(df_coint.shape[0]):
        if abs(df_coint["recent_z_score"].values[i]) > TRIGGER_Z_SCORE_THRESHOD:
            ...
            hedge_ratio = df_coint["hedge_ratio"].values[i]
            return True, symbol_1, symbol_2, hedge_ratio
