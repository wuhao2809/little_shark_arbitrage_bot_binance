...
def process_trading(symbol_1: str, symbol_2: str, hedge_ratio: float, original_z_score: float, num_wave: int):
    ...
    while not exit_flag:
        ...
        trading_flag = check_trading_status(symbol_1, symbol_2, hedge_ratio, original_z_score, limit_end_trading_time)
        time.sleep(1)
        
        # check time
        if datetime.datetime.now() > limit_end_trading_time:
            logger.critical("Limited time reached. Close all the positions.")
            trading_flag = "exit"
        
        # enter the market
        if trading_flag == "enter":
            trading_flag = quick_open_positions_market(symbol_1, symbol_2, hedge_ratio, original_z_score)
        
        # wait for trading oppotunities
        if trading_flag == "wait":
            trading_flag = wait_trade_oppotunity(symbol_1, symbol_2, hedge_ratio, original_z_score)
