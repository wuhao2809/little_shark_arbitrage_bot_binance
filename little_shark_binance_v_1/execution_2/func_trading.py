...
def get_current_z_score_dynamic(symbol_1: str, symbol_2: str, hedge_ratio):
    """
    ...
    Parameters:
        ...
        hedge_ratio: The hedge ratio used in calculating the spread.

    Returns:
        float: The current z-score for the symbol pair.
    """
    
    series_1 = binance_get_recent_close_price(symbol=symbol_1, interval=INTERVAL, limit=Z_SCORE_WINDOW)
    series_2 = binance_get_recent_close_price(symbol=symbol_2, interval=INTERVAL, limit=Z_SCORE_WINDOW)
...
# print(get_current_z_score_dynamic("OCEANUSDT", "OPUSDT", 0.08))

def check_trading_status(symbol_1, symbol_2, hedge_ratio, original_z_score, limit_end_trading_time):
    ...
    Args:
        ...
        hedge_ratio (float): The hedge ratio for the pair.
    ...
    # check z score
    if check_differnet_signal(original_z_score, current_z_score:=get_current_z_score_dynamic(symbol_1, symbol_2, hedge_ratio)):
...
def wait_trade_oppotunity(symbol_1: str, symbol_2: str, hedge_ratio: float, original_z_score: float):
    ...
    while (datetime.datetime.now() < end_time):
        current_z_score = get_current_z_score_dynamic(symbol_1, symbol_2, hedge_ratio)
