...
from config import config, WAIT_SEARCH_BEST_PAIR, TIMES_SEARCH_BEST_PAIR
...
def process_get_target_symbols_dynamic(num_wave:int) -> tuple:
    ...
    for _ in range(TIMES_SEARCH_BEST_PAIR):
        ...
        logger.info(f"Deriving recent price, with the interval of {config.INTERVAL} and num of interval limit {config.NUM_INTERVAL_LIMIT}")
