...
import pandas as pd
from config import config, TRADING_TIME_LIMIT_INTERVALS, BACKTEST_INTERVAL
...
def plot_reference(sym_1, sym_2, num_wave=0):
    ...
    """
    with open(f"{config.INTERVAL}_price_list.json", "r")as price_data:
        data = json.load(price_data)
        price_symbol_1 = data[sym_1]
        price_symbol_2 = data[sym_2]
    
    
    spread, hedge_ratio_list = calculate_spread_hedge_ratio_window(price_symbol_1, price_symbol_2, config.SPREAD_WINDOW)
    z_score = calculate_z_score_window(spread, config.Z_SCORE_WINDOW)
    
    # Make the starting point of the graph SPREAD_WINDOW + Z_SCORE_WINDOW
    price_symbol_1 = price_symbol_1[-config.NUM_INTERVAL_LIMIT:]
    price_symbol_2 = price_symbol_2[-config.NUM_INTERVAL_LIMIT:]
    
    spread = spread[-config.NUM_INTERVAL_LIMIT:]
    z_score = z_score[-config.NUM_INTERVAL_LIMIT:]
    ...
    axs[2].plot(z_score)
    axs[2].axhline(y=config.TRIGGER_Z_SCORE_THRESHOD, color='r', linestyle='dotted')
    axs[2].axhline(y=-config.TRIGGER_Z_SCORE_THRESHOD, color='r', linestyle='dotted')
...
def plot_reference_trading(symbol_1, symbol_2, num_wave=0):
    ...
    Parameters:
        ...
        num_wave (int, optional): The number of waves (used for saving the graph with a unique name). Default is 0.
    """
    price_symbol_1 = binance_get_recent_close_price(symbol_1, config.INTERVAL, config.SPREAD_WINDOW + config.Z_SCORE_WINDOW + BACKTEST_INTERVAL)
    price_symbol_2 = binance_get_recent_close_price(symbol_2, config.INTERVAL, config.SPREAD_WINDOW + config.Z_SCORE_WINDOW + BACKTEST_INTERVAL)
