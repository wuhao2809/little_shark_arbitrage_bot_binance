from config import (config, STOP_LOSS_VALUE, TAKE_PROFIT_VALUE, TOTAL_INVESTABLE_VALUE, 
                    INVESTIBLE_CAPITAL_EACH_TIME, INVESTED_VALUE_BIAS_RATIO, WAITING_INTERVAL,
                    SECONDS_WAIT_MARKET_CLOSE, SECONDS_WAIT_LIMIT_OPEN, SECONDS_WAIT_MARKET_OPEN,
...
def get_current_z_score_dynamic(symbol_1: str, symbol_2: str):
    ...
    series_1 = binance_get_recent_close_price(symbol=symbol_1, interval=config.INTERVAL, limit=config.SPREAD_WINDOW + config.Z_SCORE_WINDOW + 1)
    series_2 = binance_get_recent_close_price(symbol=symbol_2, interval=config.INTERVAL, limit=config.SPREAD_WINDOW + config.Z_SCORE_WINDOW + 1)
    
    spread, hedge_ratio_list = calculate_spread_hedge_ratio_window(series_1, series_2, config.SPREAD_WINDOW)
    zscore_series = calculate_z_score_window(spread, window=config.Z_SCORE_WINDOW)
...
def get_current_hedge_ratio_dynamic(symbol_1: str, symbol_2: str):
    ...
    """
    series_1 = binance_get_recent_close_price(symbol=symbol_1, interval=config.INTERVAL, limit=config.SPREAD_WINDOW + 1)
    series_2 = binance_get_recent_close_price(symbol=symbol_2, interval=config.INTERVAL, limit=config.SPREAD_WINDOW + 1)
...
def check_trading_status(symbol_1, symbol_2, original_z_score, limit_end_trading_time, trading_interval_count):
    ...
    # ENTER
    if (abs(current_z_score) > config.TRIGGER_Z_SCORE_THRESHOD) and (abs(current_z_score + original_z_score) == abs(current_z_score) + abs(original_z_score)):
        return "enter"
...
def wait_trade_oppotunity(symbol_1: str, symbol_2: str, original_z_score: float):
    ...
    end_time = datetime.datetime.now() + datetime.timedelta(minutes=config.INTERVAL_INT)
