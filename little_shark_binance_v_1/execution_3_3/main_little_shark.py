from config import WAVE_LIMIT, LEVERAGE, config
...
def main():
    """
    Little Shark v3.3 - Automated Trading Strategy Main Function.
    ...
    """
    # Initialization
    logger.critical("Little Shark v3.3 start!!!")
    ...
    while(num_wave <= WAVE_LIMIT):
        try:
            ...
            if status == "Searching Trading Pairs":
                ...
                logger.critical(f"Current parameters, please check: interval: {config.INTERVAL}, training_period: {config.NUM_INTERVAL_LIMIT}\n"
