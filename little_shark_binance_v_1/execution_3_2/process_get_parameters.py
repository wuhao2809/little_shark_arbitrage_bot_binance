...
from config import TRADING_TIMES_THRESHOD, INVESTIBLE_CAPITAL_EACH_TIME, TRADING_FEE_RATE, BACKTEST_INTERVAL, STOP_LOSS_VALUE, WIN_RATE_THRESHOD_DYNAMIC
...
def get_cointegrated_pairs(prices, interval, trainning_period, spread_window, z_score_window, z_score_threshod) -> str:
    ...
    for sym_1 in found_pair_list:
        ...
        for sym_2 in found_pair_list[loop_count:]:
            ...
            series_1 = series_1[:len(series_1) - BACKTEST_INTERVAL]
            series_2 = series_2[:len(series_2) - BACKTEST_INTERVAL]
            
            # Get recent NUM_LIMITS prices.
            series_1_coint_test = series_1[- trainning_period:]
            series_2_coint_test = series_2[- trainning_period:]
            ...
            if (coint_flag == 1) and (hedge_ratio > 0.001) and (hedge_ratio < 1000):
                ...
                #                                                                                                                               z_score_window,
                #                                                                                                                               z_score_threshod)
                # # Filter through static performance
                # if cumulative_returns > 0 and win_rate > 0.5 and abs(peak_loss) < 100:
                    series_1_real_test = series_1[-(BACKTEST_INTERVAL + spread_window + z_score_window):]
                    series_2_real_test = series_2[-(BACKTEST_INTERVAL + spread_window + z_score_window):]
                    ...
                    series_1_examine_test = prices[sym_1][-(BACKTEST_INTERVAL + spread_window + z_score_window):]
                    series_2_examine_test = prices[sym_2][-(BACKTEST_INTERVAL + spread_window + z_score_window):]
...
def get_trainning_result(df_coint: pd.DataFrame):
    ...
    df_coint = df_coint[abs(df_coint["peak_loss_performace"]) < STOP_LOSS_VALUE]
    df_coint = df_coint[df_coint["win_rate_performance"] > WIN_RATE_THRESHOD_DYNAMIC]
    
    # pick smallest 2/3 based on peak loss
    df_coint = df_coint.sort_values("peak_loss_performace", ascending=True).head(int(df_coint.shape[0] * (3/4)) + 1)
    # pick top 2/3 based on win rate
    df_coint = df_coint.sort_values("win_rate_performance", ascending=False).head(int(df_coint.shape[0] * (3/4)) + 1)
    # pick top 2/3 based on trade oppotunities
    df_coint = df_coint.sort_values("trade_oppotunities_performance", ascending=False).head(int(df_coint.shape[0] * (3/4)) + 1)
...
def select_parameters(df: pd.DataFrame):
    ...
    df = df[df["test_win_rate"] > WIN_RATE_THRESHOD_DYNAMIC]
    ...
    interval = str(df["interval"].values[0])

    dict = {"interval": interval, "trainning_period": trainning_period, 
            "spread_window": spread_window, "z_score_window": z_score_window, 
            "z_score_threshod": z_score_threshod,
